...
namespace NeuralNetwork
{
	public static class Utilities
	{
		...
		public static void RunEpochs(Network network, DataPoint[] data, int epochs, int logEvery = 100)
		{
			...
			for (int i = 1; i < epochs + 1; i++)
			{
				...
				if (i % logEvery == 0 || i == epochs)
				{
					...
					network.Serialize(new FileInfo("../WebSocketServer/network.nn"));
				}
			}
			Console.WriteLine();
		}
		...
		public static DataPoint[] ReadDataPoints(string fileName)
		{
            string fileContent = File.ReadAllText($"../NeuralNetwork/{fileName}");
            DataPoint[] AllDataPoints = JsonConvert.DeserializeObject<DataPoint[]>(fileContent);
